...
/**Difference between OnSharedPreferenceChangeListener and OnPreferencechangeListener
 * is that "OnPreferencechangeListener" is triggered before value try to store in the SharedPreferences file
 * so that we can validate that what user is Try to save, it's kind of validation... and
 * "OnSharedPreferenceChangeListener" will trigger only after the SharedPreferences file being modified
 * so that we can handle the UI/ or any other functonality according to SharedPreferecns file.**/
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    //SharedPreferences is the class which provide us methods to access, read, update the Data, so this is the file in which all Preferences Key-values are stored.
    @Override
    ...
    /**This method run when user taps on ok and before the value being stored into the
     * SharedPreferences file. so that we can validate the Value user is try to solve according to us.
     * because we don't trust user.**/
    @Override
